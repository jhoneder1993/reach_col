...
alojamiento_checks_new <- function(base_datos){
  ...
  dejar <- c("uuid", "encuestador", "fecha_encuesta", "organizacion", "departamento", "municipio",
             "zona_municipio", "barrio", "vereda", "sexo", "tipo_alojamiento", "num_cuartos_priv",
             "num_cuartos_comp", "cuarto_prva", "cuarto_compt")
  ...
  for (i in 1:nrow(org)) {
    filtro <- base_datos %>% filter(org[[1]][i] %in% organizacion)
